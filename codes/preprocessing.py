...
lem_b_words = lemma(lower_b_words)
lem_rf_words = lemma(lower_rf_words)

#count the apparence frequency of all the noun 
from collections import Counter        
b_words_freq = Counter(lem_b_words)
rf_words_freq = Counter(lem_rf_words)

def remove_key(d):
    for key in list(d.keys()):
        if (key.isalpha() == False) or (len(key)<2):
            d.pop(key)
        if (d[key] > 20000) or (d[key] <1000 ):
            del a[k]
